...
try {
  const { rows } = await pool.query("SELECT id, rol FROM usuarios WHERE estado = true AND rol IN ('jefe','admin','usuario') ORDER BY id");
  const jefe = rows.find(u => u.rol === 'jefe');
  if (!jefe) throw new Error('No existe jefe activo para comprobar lecturas');
  const token = jwt.sign({ id: jefe.id }, config.jwtSecret, { expiresIn: '1m' });
  ...
  paths.push('/api/socios/cuentas/disponibles','/api/comunidad/avisos','/api/comunidad/mensajes','/api/reservas','/api/importaciones/historial','/api/publico/contacto');
  ...
  if (usuario) {
    const r = await fetch(base + `/api/solicitudes/usuario/${usuario.id}`, { headers: { Authorization: `Bearer ${jwt.sign({ id: usuario.id }, config.jwtSecret, { expiresIn: '1m' })}` } });
    ...
    for (const path of ['/api/portal/cuotas','/api/portal/pagos','/api/reservas','/api/comunidad/avisos']) {
      const response=await fetch(base+path,{headers:{Authorization:`Bearer ${jwt.sign({id:usuario.id},config.jwtSecret,{expiresIn:'1m'})}`}});
      if(!response.ok)throw new Error(`HTTP ${response.status}: ${path}`);
